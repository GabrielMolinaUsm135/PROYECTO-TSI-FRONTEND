...
export default function EditarPrestamoInstrumento() {
  const { id } = useParams();
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        const listaInst = await getListaInstrumentos();
        if (!mounted) return;
        ...
        console.error('Error loading prestamo instrumento', err);
      } finally {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      payload.cod_instrumento = form.cod_item;

      const res = await actualizarPrestamoInstrumento(form.cod_prestamo, payload);
  ...
  return (
    <div className="container py-4">
      <h2>Editar Préstamo Instrumento</h2>
      <form onSubmit={handleSubmit} className="mt-3">
        <div className="row">
          ...
          <div className="col-md-6 mb-3">
            <label className="form-label">Código Instrumento</label>
